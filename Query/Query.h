...
* Template class query is implemented to accept generic type and thus PayLoad.
...
public:
	...
	Query<T> where(std::string searchString){
		...
		//std::cout << "search string is " << searchString << "\n";
		bool matched = false;
		Keys keylistResult;
		...
		for (std::vector<std::string>::iterator itkeys = keylistStart.begin(); itkeys != keylistStart.end(); ++itkeys)
		{
			if (field == 6)
				...
				case 3: matched = regEx(dbe.descrip(), conditionMatch); break;
				case 4: matched = regEx(dbe.payLoad(), conditionMatch); break;
				case 5: matched = regEx(dbe.children(), conditionMatch); break;
				default: break;
				}
			}
			if (matched == true)
			{
				//std::cout << "\nresult is matched for key " << *itkeys << "\n";
